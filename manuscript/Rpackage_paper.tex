...
To compute a Bayes factor, the core functions require the observed counts, the informed hypothesis, the parameters of the prior distribution under \(\mathcal{H}_e\), and the category labels. An overview of these arguments are provided in Table \ref{table:arguments}.
...
The package also includes S3 methods that, among other things, summarize the results, plot the parameter estimates under \(\mathcal{H}_e\), or extract the Bayes factors. Table \ref{table:s3_methods} summarizes all S3 methods currently available in \textbf{multibridge}.
...
\NormalTok{a }\OtherTok{\textless{}{-}}  \FunctionTok{rep}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{9}\NormalTok{)}
...
\NormalTok{pr2 }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\DecValTok{1}\SpecialCharTok{/}\DecValTok{9}\NormalTok{, }\DecValTok{9}\NormalTok{)}
...
\begin{verbatim}
## Warning in .checkProbability(p = p, x = x): Parameters have been rescaled.
\end{verbatim}
...
Detailed follow-up analyses are needed to discover why the data follow a monotonically decreasing pattern but not any of the two specific patterns that were put to the test (M. J. Nigrini, 2019).
...
\NormalTok{a }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{8}\NormalTok{)}
